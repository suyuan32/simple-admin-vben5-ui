...
import type { PreOrderInfo } from '#/api/payment/model/preOrderModel';
import type { ActionItem } from '#/components/table/table-action';
import type { VbenFormProps } from '@vben/common-ui';

import { useVbenVxeGrid } from '#/adapter/vxe-table';
import { deletePreOrder, getPreOrderList } from '#/api/payment/preOrder';
...
import PreOrderForm from './form.vue';
...
const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: PreOrderForm,
});
...
const gridOptions: VxeGridProps<PreOrderInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deletePreOrder({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t('payment.preOrder.addPreOrder') }}
        </Button>
      </template>
    </Grid>
  </Page>
